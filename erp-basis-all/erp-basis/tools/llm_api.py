...
def create_llm_client(provider="ollama"):
    if provider == "openai":
        ...
        return OpenAI(
            base_url="http://11.0.237.26:8090",
            api_key="not-needed"
        )
    else:
        raise ValueError(f"Unsupported provider: {provider}")

def query_llm(prompt: str, client=None, model=None, provider="ollama", image_path: Optional[str] = None) -> Optional[str]:
    """
...
def main():
    ...
    parser.add_argument('--provider', choices=['openai','anthropic','gemini','local','deepseek','azure','ollama'], default='ollama', help='The API provider to use')
